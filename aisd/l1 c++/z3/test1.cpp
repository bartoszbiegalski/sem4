...
int cost(CycleLine *cycleLine, int value, bool direction);
...
int main() {
    ...
    std::uniform_int_distribution<std::mt19937::result_type> dist3(0,1); // distribution in range [1, 6]
    ...
    for(int i = 0; i < 1000; i++) {
        int x = dist2(rng);
        average1 = (double) cost(randomCycleLine, T[x], true) + average1;
    }
    average1 = average1 / 1000.0;

    std::cout<<"First case: "<<average1<<std::endl;

    /// case 2: 1000 total random numbers between 1 and 100 000, but now we go left or right based on 0 or 1
    bool left = false;
    if(dist3(rng) == 1) {
        left = true;
    }
    ...
    for(int i = 0; i < 1000; i++) {
        int x = dist1(rng);
        average2 = (double) cost(randomCycleLine, x, left) + average2;
    }
...
void insert(CycleLine *cycleLine, int value) {
    ...
    if (cycleLine->getStart() == NULL) {
        ...
        cycleLine->getStart()->setPrevious(cycleLine->getStart());
        ...
        cycleLine->getStart()->getNext()->setPrevious(cycleLine->getStart());

        cycleLine->getStart()->getNext()->setNext(node);
        node->setPrevious(cycleLine->getStart()->getNext());
...
void print(CycleLine *cycleLine) {
    ...
    while (size >= 0) {
        //std::cout<<node->getPrevious()->getValue()<<" <- "<<node->getValue()<<" -> "<<node->getNext()->getValue()<<std::endl;
...
CycleLine* merge(CycleLine *cycleLine1, CycleLine *cycleLine2) {
    if(cycleLine1->getStart() == nullptr && cycleLine2->getStart() == nullptr) {
        ...
        cycleLine2->getStart()->setPrevious(cycleLine1->getStart()); /// connected the back pointer of second line to first of first line
